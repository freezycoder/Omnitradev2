...
from providers.macro.kcroro_client import KcroroSeriesBundle
...
RORO_FEATURES = ("kcroro_level", "kcroro_shock_sum_20d")
RORO_NON_EQUITY_FEATURES = ("kcroro_spreads", "kcroro_liquidity", "kcroro_fxgold")
MIN_FOLD_DATES = 4
MIN_FOLD_HITS = 12
...
GateArm = Literal[
    ...
    "roro_only",
    "hy_plus_nfci",
    "hy_plus_nfci_plus_roro",
    "roro_non_equity",
    "hy_plus_nfci_plus_roro_non_equity",
]
GATE_ARMS: tuple[GateArm, ...] = (
    "ungated",
    "hy_oas_only",
    "nfci_only",
    "roro_only",
...
class LongScreenObservation:
    ...
    kcroro_level: float | None = None
    kcroro_shock_sum_20d: float | None = None
    kcroro_spreads: float | None = None
    kcroro_liquidity: float | None = None
    kcroro_fxgold: float | None = None
    hy_gate: GateStatus = "unknown"
    hy_weight: float | None = None
    nfci_gate: GateStatus = "unknown"
    nfci_weight: float | None = None
    roro_gate: GateStatus = "unknown"
    roro_weight: float | None = None
    ablation_gate: GateStatus = "unknown"
    ablation_weight: float | None = None
...
class KcroroRegimeEvalService:
    """Offline KCRORO nested-gate harness. Never writes live scores or recommendations."""
    ...
    def evaluate_panels(
        ...
        abort_reasons = list(kcroro_panel.abort_reasons)
        ...
        ablation_available = any(row.ablation_gate != "unknown" for row in resolved_hits)
        ...
        view = build_kcroro_regime_view(kcroro_panel)
        ...
        return {
            ...
            "deployment_guard": {
                ...
                "message": (
                    "KCRORO is a shadow risk-on/off regime gate that throttles "
            ...
            "pre_registration": {
                ...
                "success_rule": (
                    "KCRORO adds incremental gated performance (HY+NFCI+RORO vs "
                    "HY+NFCI) on Sharpe or max drawdown, or incremental nested IC/R² "
                    "versus HY+NFCI, in at least 2 of 3 walk-forward folds, AND the "
                    "equity-leg ablation still leaves non-zero lift from spreads/"
                    "funding/FX-gold."
                ),
                "kill_switch": (
                    "KILL as redundant with KEEP HY OAS + NFCI when mean incremental "
                    f"IC and R² versus HY+NFCI sit inside ±{REDUNDANCY_IC_EPSILON} / "
                    f"±{REDUNDANCY_R2_EPSILON} and HY+NFCI+RORO shows no primary-metric "
                    "gated lift versus HY+NFCI; OR KILL the equity leg if headline lift "
                    "vanishes after dropping KCROROE."
                ),
                "gate_is_fitted": False,
                "subindexes_in_headline_gate": False,
            },
            "citation": KCRORO_CITATION,
            "ice_redistribution_notice": ICE_REDISTRIBUTION_NOTICE,
            "kcroro_panel": kcroro_panel.to_dict(),
    ...
    def evaluate_bundle(
        ...
        *,
        ...
        kcroro_panel = build_kcroro_panel(bundle, spy_history=spy_history, vix_history=vix_history)
    ...
    def _with_panel_features(
        ...
        for hit in hits:
            roro_snapshot = kcroro_panel.snapshot_on(hit.signal_date)
            hy_snapshot = hy_panel.snapshot_on(hit.signal_date)
            nfci_snapshot = nfci_panel.snapshot_on(hit.signal_date)
            roro_gate: GateStatus = roro_snapshot.gate.status if roro_snapshot else "unknown"
            ablation_gate: GateStatus = roro_snapshot.ablation_gate.status if roro_snapshot else "unknown"
            ...
            joined.append(
                replace(
                    ...
                    spy_realized_vol_20d=(
                        roro_snapshot.spy_realized_vol_20d
                        if roro_snapshot and roro_snapshot.spy_realized_vol_20d is not None
                        else hit.spy_realized_vol_20d
                    ),
                    vix_level=(
                        roro_snapshot.vix_level
                        if roro_snapshot and roro_snapshot.vix_level is not None
                        else hit.vix_level
                    ),
                    ...
                    kcroro_level=roro_snapshot.kcroro if roro_snapshot else hit.kcroro_level,
                    kcroro_shock_sum_20d=(
                        roro_snapshot.shock_sum_20d if roro_snapshot else hit.kcroro_shock_sum_20d
                    ),
                    kcroro_spreads=roro_snapshot.spreads if roro_snapshot else hit.kcroro_spreads,
                    kcroro_liquidity=roro_snapshot.liquidity if roro_snapshot else hit.kcroro_liquidity,
                    kcroro_fxgold=roro_snapshot.fx_gold if roro_snapshot else hit.kcroro_fxgold,
                    hy_gate=hy_gate,
                    ...
                    roro_gate=roro_gate,
                    roro_weight=roro_snapshot.gate.throttle_weight if roro_snapshot else None,
                    ablation_gate=ablation_gate,
                    ablation_weight=roro_snapshot.ablation_gate.throttle_weight if roro_snapshot else None,
                )
            )
        return joined
    ...
    def _data_quality(
        ...
        *,
        ...
        return {
            ...
            "roro_full_hits": sum(row.roro_gate == "full" for row in hits),
            "roro_throttle_hits": sum(row.roro_gate == "throttle" for row in hits),
            "roro_unknown_hits": sum(row.roro_gate == "unknown" for row in hits),
            "hy_full_hits": sum(row.hy_gate == "full" for row in hits),
            "nfci_full_hits": sum(row.nfci_gate == "full" for row in hits),
            "ablation_known_hits": sum(row.ablation_gate != "unknown" for row in hits),
            ...
            "ablation_available": ablation_available,
            "kcroro_history_sessions": kcroro_panel.history_sessions,
            "series_source": kcroro_panel.source,
    ...
    def _evaluate_gate_fold(
        ...
        *,
        ...
        if len(books["roro_only"]) < MIN_GATED_FOLD_DAYS:
            return _empty_gate_fold(fold, validation, abort_reason="KCRORO-gated validation book is too small.")
        ...
        keep_combo_ready = hy_available and nfci_available
        return {
            "fold": fold.fold,
            "eligible": True,
            ...
            "roro_only": metrics["roro_only"],
            "hy_plus_nfci": metrics["hy_plus_nfci"] if keep_combo_ready else None,
            "hy_plus_nfci_plus_roro": metrics["hy_plus_nfci_plus_roro"] if keep_combo_ready else None,
            "roro_non_equity": metrics["roro_non_equity"] if ablation_available else None,
            "hy_plus_nfci_plus_roro_non_equity": (
                metrics["hy_plus_nfci_plus_roro_non_equity"]
                if keep_combo_ready and ablation_available
                else None
            ),
            "lift_roro_vs_ungated": _metric_lift(metrics["roro_only"], metrics["ungated"]),
            "lift_hy_plus_nfci_plus_roro_vs_hy_nfci": (
                _metric_lift(metrics["hy_plus_nfci_plus_roro"], metrics["hy_plus_nfci"])
                if keep_combo_ready
                else None
            ),
            "lift_ablation_vs_hy_nfci": (
                _metric_lift(metrics["hy_plus_nfci_plus_roro_non_equity"], metrics["hy_plus_nfci"])
                if keep_combo_ready and ablation_available
                else None
            ),
    ...
    def _evaluate_nested_fold(
        ...
        *,
        ...
        baseline_names.extend(_usable_features(train_rows, VIX_FEATURES))
        ...
        roro_features = _usable_features(train_rows, RORO_FEATURES)
        ablation_features = (
            _usable_features(train_rows, RORO_NON_EQUITY_FEATURES) if ablation_available else []
        )
        if not baseline_features:
            return _empty_nested_fold(
                ...
                train_rows,
                validation_rows,
                abort_reason="Equity-vol (and VIX, if shipped) features are unavailable in training.",
            )
        if not roro_features:
            return _empty_nested_fold(
                fold,
                train_rows,
                validation_rows,
                used_baseline_features=tuple(baseline_features),
                abort_reason="KCRORO features are unavailable in the training window.",
            )
        if hy_available and not hy_features:
            return _empty_nested_fold(
                ...
                validation_rows,
                used_baseline_features=tuple(baseline_features),
                used_roro_features=tuple(roro_features),
                abort_reason="KEEP HY OAS features are unavailable in the training window.",
            )
        if nfci_available and not nfci_features:
            return _empty_nested_fold(
                fold,
                train_rows,
                validation_rows,
                used_baseline_features=tuple(baseline_features),
                used_roro_features=tuple(roro_features),
                abort_reason="KEEP NFCI features are unavailable in the training window.",
            )
        ...
        roro_pred = _predict_ols(
            train_rows,
            validation_rows,
            (*baseline_features, *roro_features),
            y_train,
        )
        hy_pred = (
        ...
        nfci_pred = (
            _predict_ols(train_rows, validation_rows, (*baseline_features, *nfci_features), y_train)
            if nfci_features
            else None
        )
        keep_pred = (
            ...
            if hy_features and nfci_features
            else None
        )
        both_pred = (
            _predict_ols(
                train_rows,
                validation_rows,
                (*baseline_features, *hy_features, *nfci_features, *roro_features),
                y_train,
            )
            if hy_features and nfci_features
            else None
        ...
        ablation_pred = (
            _predict_ols(
                train_rows,
                validation_rows,
                (*baseline_features, *hy_features, *nfci_features, *ablation_features),
                y_train,
            )
            if hy_features and nfci_features and ablation_features
            else None
        )
        baseline_metrics = _prediction_metrics(baseline_pred, realized)
        roro_metrics = _prediction_metrics(roro_pred, realized)
        hy_metrics = _prediction_metrics(hy_pred, realized) if hy_pred is not None else None
        nfci_metrics = _prediction_metrics(nfci_pred, realized) if nfci_pred is not None else None
        ...
        ablation_metrics = (
            _prediction_metrics(ablation_pred, realized) if ablation_pred is not None else None
        )
        lift_roro_vs_keep = (
            {
        ...
        lift_ablation_vs_keep = (
            {
                key: _subtract(ablation_metrics[key], keep_metrics[key])
                for key in ("spearman_ic", "r_squared")
            }
            if ablation_metrics is not None and keep_metrics is not None
            else {"spearman_ic": None, "r_squared": None}
        )
        return {
            "fold": fold.fold,
            ...
            "roro_only": roro_metrics,
            "hy_plus_nfci": keep_metrics,
            "hy_plus_nfci_plus_roro": both_metrics,
            "roro_non_equity": ablation_metrics,
            "lift_roro_vs_hy_nfci": lift_roro_vs_keep,
            "lift_ablation_vs_hy_nfci": lift_ablation_vs_keep,
            "redundant_with_hy_nfci": _is_redundant(lift_roro_vs_keep) if hy_available and nfci_available else False,
            "used_baseline_features": list(baseline_features),
            "used_hy_features": list(hy_features),
            "used_nfci_features": list(nfci_features),
            "used_roro_features": list(roro_features),
            "used_ablation_features": list(ablation_features),
            "hy_keep_available": hy_available,
            "nfci_keep_available": nfci_available,
            "ablation_available": ablation_available,
            "abort_reason": None,
        ...
        *,
        ...
        gated_metric_lifts = {
            metric: sum(
                ...
                if (row.get("lift_hy_plus_nfci_plus_roro_vs_hy_nfci") or {}).get(metric) is not None
                and float(row["lift_hy_plus_nfci_plus_roro_vs_hy_nfci"][metric]) > 0
            )
            for metric in PRIMARY_METRICS
        }
        winning_gated_metrics = [metric for metric, wins in gated_metric_lifts.items() if wins >= 2]
        nested_ic_lifts = [
            float(row["lift_roro_vs_hy_nfci"]["spearman_ic"])
            for row in eligible_nested
            if row.get("lift_roro_vs_hy_nfci", {}).get("spearman_ic") is not None
        ]
        nested_r2_lifts = [
            float(row["lift_roro_vs_hy_nfci"]["r_squared"])
            for row in eligible_nested
            if row.get("lift_roro_vs_hy_nfci", {}).get("r_squared") is not None
        ]
        nested_positive_folds = sum(
            1
            for row in eligible_nested
            if _positive_incremental(row.get("lift_roro_vs_hy_nfci") or {})
        )
        mean_nested_ic_lift = (
        ...
        headline_success = bool(winning_gated_metrics or incremental_explanatory)
        ablation_gated_wins = {
            metric: sum(
                1
                for row in eligible_gate
                if (row.get("lift_ablation_vs_hy_nfci") or {}).get(metric) is not None
                and float(row["lift_ablation_vs_hy_nfci"][metric]) > 0
            )
            for metric in PRIMARY_METRICS
        }
        ablation_nested_positive = sum(
            1
            for row in eligible_nested
            if _positive_incremental(row.get("lift_ablation_vs_hy_nfci") or {})
        )
        ablation_has_lift = any(wins >= 2 for wins in ablation_gated_wins.values()) or ablation_nested_positive >= 2
        reasons = list(abort_reasons)
        kill_switch = "not_triggered"
        if reasons:
            outcome = "aborted"
        elif not hy_available or not nfci_available:
            outcome = "aborted"
            reasons.append(
                "KEEP HY OAS + NFCI comparators are unavailable; the nested kill switch "
                "versus credit-gate-v1 and nfci-gate-v1 cannot be scored."
            )
        ...
        elif not ablation_available:
            outcome = "aborted"
            reasons.append(
                "Equity-leg ablation cannot be scored because KCROROS/L/G are missing. "
                "SUCCESS requires non-zero lift after dropping the equity subindex."
            )
        elif redundant and not winning_gated_metrics:
            outcome = "fail"
            kill_switch = "kill_redundant_with_hy_nfci"
            reasons.append(
                "KILL: KCRORO adds no incremental R² / IC versus KEEP HY OAS + NFCI "
                f"(mean IC lift {mean_nested_ic_lift}, mean R² lift {mean_nested_r2_lift}) "
                "and HY+NFCI+RORO shows no gated lift versus HY+NFCI."
            )
        elif headline_success and not ablation_has_lift:
            outcome = "fail"
            kill_switch = "kill_equity_leg_circular"
            reasons.append(
                "KILL equity leg: headline KCRORO lift versus HY+NFCI does not survive "
                "dropping KCROROE. Residual spreads/funding/FX-gold lift is zero, so the "
                "result is circular with SPX."
            )
        elif headline_success and ablation_has_lift:
            outcome = "success"
            reasons.append(
                "KCRORO added incremental gated performance and/or nested explanatory "
                "power versus KEEP HY OAS + NFCI in at least 2 of 3 folds, and the "
                "equity-leg ablation still left non-zero lift from spreads/funding/FX-gold."
            )
        else:
            outcome = "fail"
            kill_switch = "kill_no_incremental_lift"
            reasons.append(
                "FAIL: KCRORO did not improve HY+NFCI+RORO gated Sharpe/max drawdown versus "
                "HY+NFCI in at least 2 of 3 folds, and nested incremental IC/R² did not "
                "clear the redundancy band in at least 2 folds."
            )
        return {
            "outcome": outcome,
            ...
            "positive_hy_nfci_plus_roro_lift_folds_by_metric": gated_metric_lifts,
            "winning_primary_metrics": winning_gated_metrics,
            "nested_positive_incremental_folds": nested_positive_folds,
            "mean_nested_ic_lift_vs_hy_nfci": mean_nested_ic_lift,
            "mean_nested_r2_lift_vs_hy_nfci": mean_nested_r2_lift,
            "kcroro_redundant_with_hy_nfci": redundant,
            "ablation_has_lift": ablation_has_lift,
            "ablation_gated_lift_folds_by_metric": ablation_gated_wins,
            "ablation_nested_positive_folds": ablation_nested_positive,
            "hy_keep_available": hy_available,
            "nfci_keep_available": nfci_available,
            "ablation_available": ablation_available,
...
def _daily_book(hits: Sequence[LongScreenObservation], *, mode: GateArm) -> dict[date, float]:
    ...
    for as_of, rows in grouped.items():
        ...
        match mode:
            ...
            case "nfci_only":
                ...
                book[as_of] = weight * mean_return
            case "roro_only":
                weight = _gate_weight(sample.roro_gate, sample.roro_weight)
                if weight is None:
                    continue
                book[as_of] = weight * mean_return
            case "hy_plus_nfci":
            ...
            case "hy_plus_nfci_plus_roro":
                hy_weight = _gate_weight(sample.hy_gate, sample.hy_weight)
                nfci_weight = _gate_weight(sample.nfci_gate, sample.nfci_weight)
                roro_weight = _gate_weight(sample.roro_gate, sample.roro_weight)
                if hy_weight is None or nfci_weight is None or roro_weight is None:
                    continue
                book[as_of] = hy_weight * nfci_weight * roro_weight * mean_return
            case "roro_non_equity":
                weight = _gate_weight(sample.ablation_gate, sample.ablation_weight)
                if weight is None:
                    continue
                book[as_of] = weight * mean_return
            case "hy_plus_nfci_plus_roro_non_equity":
                hy_weight = _gate_weight(sample.hy_gate, sample.hy_weight)
                nfci_weight = _gate_weight(sample.nfci_gate, sample.nfci_weight)
                ablation_weight = _gate_weight(sample.ablation_gate, sample.ablation_weight)
                if hy_weight is None or nfci_weight is None or ablation_weight is None:
                    continue
                book[as_of] = hy_weight * nfci_weight * ablation_weight * mean_return
...
def _empty_gate_fold(
    ...
    *,
    ...
    return {
        ...
        "roro_only": empty,
        "hy_plus_nfci": None,
        "hy_plus_nfci_plus_roro": None,
        "roro_non_equity": None,
        "hy_plus_nfci_plus_roro_non_equity": None,
        "lift_roro_vs_ungated": {"sharpe": None, "max_drawdown": None},
        "lift_hy_plus_nfci_plus_roro_vs_hy_nfci": None,
        "lift_ablation_vs_hy_nfci": None,
...
def _empty_nested_fold(
    ...
    *,
    ...
    return {
        ...
        "hy_plus_nfci_plus_roro": None,
        "roro_non_equity": None,
        "lift_roro_vs_hy_nfci": {"spearman_ic": None, "r_squared": None},
        "lift_ablation_vs_hy_nfci": {"spearman_ic": None, "r_squared": None},
        ...
        "used_roro_features": list(used_roro_features),
        "used_ablation_features": [],
        "hy_keep_available": False,
        "nfci_keep_available": False,
        "ablation_available": False,
        "abort_reason": abort_reason,
