#!/usr/bin/env python3
"""Build the shadow OFR STFM panel and run the frozen nested-gate eval.
...
from application.stfm_funding_liquidity_eval_service import StfmFundingLiquidityEvalService
from config.stfm_funding_liquidity import ICE_REDISTRIBUTION_NOTICE
...
def main() -> int:
    ...
    parser.add_argument(
        "--period",
        default="2y",
        help="yfinance period for SPY (equity-vol control).",
    )
    parser.add_argument(
        "--no-public-csv",
        action="store_true",
    ...
    client = OfrStfmClient(allow_public_csv=not args.no_public_csv)
    ...
    payload: dict[str, Any] = StfmFundingLiquidityEvalService().evaluate_bundle(
        bundle,
    ...
    payload["cli"] = {
        ...
        "frozen_mnemonics": bundle.history_plan.get("frozen_mnemonics"),
        "null_counts": bundle.null_counts,
    }
